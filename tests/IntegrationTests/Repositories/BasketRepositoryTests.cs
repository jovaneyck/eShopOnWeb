...
{
    private (DbContext, BasketRepository repository) BuildFreshRepository()
    {
        var context = new CatalogContext(new DbContextOptionsBuilder<CatalogContext>()
            .UseInMemoryDatabase(databaseName: "TestCatalog")
            .Options);
        return (context, new BasketRepository(context));
    }
    ...
    public async Task CanPersistAndReadAggregate()
    {
        ...
        var retrieved = await repo.GetAggregateById(basket.Id, TestContext.Current.CancellationToken);
